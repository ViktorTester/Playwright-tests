...
// пред/пост-условия через оба блока beforeAll и afterAll.

// этот код выполнится перед началом всех test-блоков (один раз)
...
// В блоке afterAll фикстуру browser задавать не надо
// она берется из блока beforeAll

// Этот код выполнится по завершению всех test-блоков (один раз)
test.afterAll(async() => {
...
// Перед тестом выполняется блок beforeAll
...
// После теста выполняется блок afterAll