...
// .toBeChecked()
    ...
    await expect(await page.locator(np.radioGender)).toBeChecked()

// .toHaveAttribute()
    // находим элемент и проверяем, что в нем имеется атрибут с таким-то значением
    await expect(await page.locator(np.registerBtn)).toHaveAttribute('type', 'submit')

// .toHaveText()
    // находим элемент и проверяем его на точное совпадение по тексту
    await expect(await page.locator(np.pageH1Title)).toHaveText('Register')

// .toContainText()
    // находим элемент и проверяем его на частичное совпадение по тексту
    await expect(await page.locator(np.pageH1Title)).toContainText('Reg')
...
// .toHaveValue()
    ...
    await expect(await page.locator(np.firstNameInput)).toHaveValue('tester1')
...
// .toHaveCount()
    ...
    await expect(await page.locator(np.dayDropdownValues)).toHaveCount(32)
